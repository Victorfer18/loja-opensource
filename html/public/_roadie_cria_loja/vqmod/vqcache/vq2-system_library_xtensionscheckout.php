...
class XtensionsCheckout extends Controller {
	...
	public function renderView($obj) {
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/'.$obj->template)) {
			return $this->load->view($this->config->get('config_template') . '/template/'.$obj->template, $obj->data);
		} else {
			return $this->load->view('default/template/'.$obj->template, $obj->data);
		}
	}

	public function renderDirectView($file,$data){
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/'.$file)) {
			...
			return $this->load->view('default/template/'.$file, $data);
		}
	}
			...
			if($fieldcount > 1){
				...
				foreach ($field as $newkey => $newfield){					
					foreach ($newfield as $newnewkey => $newnewfield){
						...
						$field_html[$newkey][$newnewkey] = $this->renderDirectView($obj->config->get('xtensions_view_path').'makefields.tpl', $field_view);
				...
				foreach ($field as $newkey => $newfield){					
					foreach ($newfield as $newnewkey => $newnewfield){
						...
						$fields[] = $this->renderDirectView($obj->config->get('xtensions_view_path').'makefields.tpl', $field_view);
					}							
				}
			}			
		}
	...
	public function validateCustomField($custom_field,$custom_field_post){
		...
		// @formatter:off
		if (($custom_field['type'] == 'text') && !empty($custom_field['xvalidation']) && !filter_var($custom_field_post, FILTER_VALIDATE_REGEXP, array('options' => array('regexp' => $custom_field['xvalidation'])))) {
